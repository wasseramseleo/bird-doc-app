...
| `/offline-bundle/` | *(composite, not a ViewSet)* | Read-only GET for any Mitglied — the offline reference bundle (PRD #152, issue #157), scoped to the active Organisation |

### Key Non-Obvious Behaviors

**Der Fehlervertrag — `errors` beside every rejection (ADR 0038, PRD #438)** — a global `EXCEPTION_HANDLER` (`birds/errors.py`, wired in `REST_FRAMEWORK`) wraps DRF's own handler and adds the **sibling key** `errors` to every rejection body DRF renders: a list of `{field, code, detail}`, one entry per German sentence, `field: null` for a rejection of the record as a whole (`detail`, `non_field_errors`). It is **additive, never replacing** — the bodies the API has always sent (`{field: [string]}` and `{"detail": …}`) stay **byte identical**, the hard constraint from ADR 0033: a device may be offline ~30 days, so the bundle replaying a month-old payload *is* the old bundle. That the *old bundle* is genuinely unmoved by the new key is proven **in the frontend suite, against the real client** — `frontend/src/app/service/sync.service.spec.ts` replays every rejection shape with the envelope bolted on and asserts the identical `syncError` — and not by a Python transcription of `sync.service.ts::extractServerMessage`, which would pass no matter what the TypeScript did. Keep it that way: `extractServerMessage` is the one extractor in the app that enumerates **all** body keys (`Object.values`) rather than reading named ones, so it is the one that can swallow a new sibling key badly. DRF's own codes therefore ride along everywhere for free (`required`, `blank`, `invalid_choice`, `max_length`, `unique`, `permission_denied`, `not_found`, and any `APIException`'s `default_code` such as `seat_limit_reached`) — no line of code per view. Nested and list-field errors are flattened to a stable path (`parasites[1]`, `items[0].name`), never a nested object where an entry is promised. `errors` is thereby a **reserved top-level key** of a rejection body. Two rejections stay outside its reach by construction: a body that is not a mapping (`raise ValidationError("…")` outside `is_valid` renders as a JSON *list* — no sibling key to hang the envelope on, e.g. the last-Admin removal) and a rejection a view builds by hand as a plain `Response` (the IWM row cap, the login refusals in `auth_views.py`), which never passes through an exception handler at all; both are picked up by the Domänencode pass.
...
**Smart ring numbering** — `GET /api/birds/rings/next-number?size=V&project=<uuid>` returns `{"next_number": <string> | null}`: the *last consumed* number on the rope **+ 1**, never `max + 1`. It takes the most recently created (`created`) `DataEntry` of that size in the given `project` that drew a fresh number from the rope — a first catch (`bird_status='e'`) **or** a destroyed-ring record (`species.special_kind == "ring_destroyed"`); recaptures (Wiederfang) consume nothing and are excluded, and the recording Beringer is irrelevant. The whole computation is **scoped to the requester's active Organisation** (ADR 0006) — another Organisation's consumption of the same size never drives it, and an account with no active Organisation gets `null`. The numeric value is incremented while leading-zero width is preserved (`0042` → `0043`, returned as a string). It returns `null` when the Organisation/project has no qualifying capture of that size or the previous number is non-numeric — there is no global/other-project fallback (issues #22, #42). The consumption predicate itself lives in `views.py::_ring_consuming_entries()`, shared verbatim with the offline bundle below so the two never drift apart.

**Offline reference bundle (PRD #152, issue #157)** — `GET /api/birds/offline-bundle/` (`OfflineBundleView`, a plain `APIView`, not a ViewSet — it spans several models) hands a device everything it needs to cache while online to work fully offline later, in one round trip, scoped to the requester's **active Organisation**. It bundles: the **offline species pool** (the requester's active Artenliste members plus every Sonderart, always selectable, plus every species the Organisation has ever used in a capture — never the whole `Species` reference table (the ~460-row Austrian list) — each annotated with a per-Organisation `usage_count` so the offline picker can approximate `SpeciesViewSet`'s most-used-first ordering); **org reference data** (active, non-archived Stationen; the Organisation's own Beringer, excluding the reserved `GELÖSCHT` fallback; and the requester's own Projekte, scoped to their Beringer exactly as `ProjectViewSet` scopes them online); the **last-consumed (raw, un-incremented) ring number per Projekt + Ringgröße** — one entry per `(project, size)` pair that has a qualifying consumption, using the exact same rule as `next-number` (`_ring_consuming_entries`), so the client can derive its own "last consumed + 1" suggestion offline by combining it with its own queued captures; and the requester's **cached identity** (username, Beringer-Kürzel, Organisation, Rolle) for the offline auth bootstrap. With no resolvable active Organisation every collection is empty (mirrors the other org-scoped endpoints — empty, never a 403 or another tenant's data).
...
**Species filtering by `special_kind` (issue #427)** — `GET /species/?special_kind=<value>` narrows the candidates to one value of the Sonderart discriminator, so a client resolves a rare Sonderart ("Ring Vernichtet") by key instead of hoping it turns up on the usage-sorted first page. It is a **pure narrowing**: the active-Artenliste membership and the usage ordering are untouched, an **unknown value simply matches no row** (an empty page, 200 — never a 400/500), and an absent parameter leaves the endpoint exactly as it was.
...
`Species` is the Austrian species list (~460 rows) loaded from `artenliste_2024.csv` via the `0003_import_species_data` migration. It carries the recommended ring size per species, which the frontend uses to pre-fill the ring size field.
